...
def merge_live_model(catalog: dict[str, list[dict[str, Any]]], provider: str, model_id: str, live: dict[str, Any]) -> None:
    ...
    if existing is None:
        existing = {"id": model_id, "label": live.get("label") or model_id, "category": "text-to-image", "notes": live.get("description") or "Discovered from the provider model endpoint."}
