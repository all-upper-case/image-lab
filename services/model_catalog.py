...
MODEL_CATALOG: dict[str, list[dict[str, Any]]] = {
    "venice": [
        {"id": "grok-imagine-image", "label": "Grok Imagine Image", "category": "text-to-image", "sizing_mode": "mixed", "pricing": {"unit": "unknown", "source": "Venice model pricing should be checked through the live models endpoint for the current account."}, "notes": "Used in the Venice image-generation docs example. Supports the native Venice image endpoint."},
        {"id": "gpt-image-2", "label": "GPT Image 2", "category": "text-to-image", "sizing_mode": "resolution_tier", "supported_resolutions": ["1K", "2K", "4K"], "pricing": {"unit": "unknown", "source": "Venice model pricing should be checked through the live models endpoint for the current account."}, "notes": "Venice docs list this as a resolution-tier model using aspect_ratio plus resolution."},
        {"id": "nano-banana-pro", "label": "Nano Banana Pro", "category": "text-to-image", "sizing_mode": "resolution_tier", "supported_resolutions": ["1K", "2K", "4K"], "pricing": {"unit": "unknown", "source": "Venice model pricing should be checked through the live models endpoint for the current account."}, "notes": "Venice docs list this as a resolution-tier model using aspect_ratio plus resolution."},
        {"id": "nano-banana-2", "label": "Nano Banana 2", "category": "text-to-image", "sizing_mode": "resolution_tier", "supported_resolutions": ["1K", "2K", "4K"], "pricing": {"unit": "unknown", "source": "Venice model pricing should be checked through the live models endpoint for the current account."}, "notes": "Venice docs list this as a resolution-tier model using aspect_ratio plus resolution."},
        {"id": "qwen-image-2", "label": "Qwen Image 2", "category": "text-to-image", "sizing_mode": "aspect_ratio", "pricing": {"unit": "unknown", "source": "Venice model pricing should be checked through the live models endpoint for the current account."}, "notes": "Venice docs list this as an aspect-ratio model."},
        {"id": "qwen-image", "label": "Qwen Image", "category": "text-to-image", "sizing_mode": "pixel", "pricing": {"unit": "unknown", "source": "Venice model pricing should be checked through the live models endpoint for the current account."}, "notes": "Venice docs list this as a pixel-dimension model. The app supplies default dimensions when width/height are left blank."},
        {"id": "venice-sd35", "label": "Venice SD 3.5", "category": "text-to-image", "sizing_mode": "pixel", "pricing": {"unit": "unknown", "source": "Venice model pricing should be checked through the live models endpoint for the current account."}, "notes": "Venice docs list this as a pixel-dimension model. The app supplies default dimensions when width/height are left blank."},
        {"id": "fluently-xl", "label": "Fluently XL", "category": "text-to-image", "sizing_mode": "pixel", "pricing": {"unit": "unknown", "source": "Venice model pricing should be checked through the live models endpoint for the current account."}, "notes": "Starter model kept from the initial app. Confirm availability on your account."},
        {"id": "hidream", "label": "HiDream", "category": "text-to-image", "sizing_mode": "mixed", "pricing": {"unit": "unknown", "source": "Venice model pricing should be checked through the live models endpoint for the current account."}, "notes": "Starter model kept from the initial app. Confirm supported sizing in the live model metadata."},
    ],
    "fal": [
        ...
        {"id": "fal-ai/flux-2-flex", "label": "FLUX.2 Flex (verify endpoint later)", "category": "text-to-image", "fal_payload_style": "image_size", "disabled_for_text_only": True, "pricing": {"unit": "megapixel_rounded_up", "usd": 0.05, "source": "Fal model page/search result; endpoint not verified in this app yet."}, "notes": "Potential future model. Hidden from the generation dropdown until the exact Fal endpoint and schema are verified."},
    ],
}
...
def merge_live_model(catalog: dict[str, list[dict[str, Any]]], provider: str, model_id: str, live: dict[str, Any]) -> None:
    ...
    if existing is None:
        existing = {
            "id": model_id,
            "label": live.get("label") or model_id,
            "category": "text-to-image",
            "notes": live.get("description") or "Discovered from the provider model endpoint.",
        }
